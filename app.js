...
const state = {
  ...
  selectedApiId: "front-id", preset: "idCard", stream: null, ocrResult: null, warnings: [],
...
async function runOcr() {
  ...
  try {
    const file = getOcrFile(), formData = buildFormData(api, file);
...
function validateBeforeRunOcr(api) {
  ...
  if (!state.mockMode && api.authRequired && state.apiStatus[api.id]?.available === false) return "API Other ยังใช้งานไม่ได้: กรุณากำหนด OCR_OTHER_AUTH_TOKEN ในไฟล์ .env แล้วรัน uvicorn server:app --host 127.0.0.1 --port 3000 ใหม่";
  return "";
}
...
function renderApiList() {
  let group = "";
  els.apiList.innerHTML = OCR_APIS.map((api) => { const heading = api.group !== group ? `<div class="api-group">${escapeHtml(api.group)}</div>` : ""; group = api.group; const unavailable = state.apiStatus[api.id]?.available === false; return `${heading}<button class="api-option ${api.id === state.selectedApiId ? "active" : ""}" data-api-id="${api.id}" type="button"><strong>${escapeHtml(api.label)}</strong><span>${escapeHtml(api.endpoint)} · Proxy${unavailable ? " · ต้องตั้ง Token" : ""}</span></button>`; }).join("");
  els.selectedApiLabel.textContent = getSelectedApi().label;
}
...
async function normalizeOcrResponse(rawJson, api, runtime) {
  ...
  const imagePreview = await extractImagePreview(rawJson);
  return { rawJson: sanitizeOcrPayload(rawJson), plainText: directText || formatFieldsAsText(fields) || "OCR สำเร็จ แต่ไม่พบข้อความที่สามารถแสดงผลได้", fields, runtime, apiId: api.id, apiLabel: api.label, outputMode: api.outputMode, imagePreview, encodedPayloads };
}
...
async function extractImagePreview(payload) {
  const imagePayload = extractImagePayload(payload);
  if (!imagePayload) return { src: "", key: "", message: "" };
  try {
    return await decodeOcrImagePayload(imagePayload.value, imagePayload.key);
  } catch {
...
function showResultTab(tab) { const json = tab === "json"; state.resultTab = json ? "json" : "text"; els.textOutput.hidden = json; els.jsonOutput.hidden = !json; els.textTab.classList.toggle("active", !json); els.jsonTab.classList.toggle("active", json); renderEncodedPayloadPanel(); }
...
function downloadFile(content, type, extension) { const url = URL.createObjectURL(new Blob([content], { type })), link = document.createElement("a"); link.href = url; link.download = `ocr-result-${new Date().toISOString().slice(0, 10)}.${extension}`; link.click(); URL.revokeObjectURL(url); }
